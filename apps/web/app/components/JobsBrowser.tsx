...
type Job = {
    id:number; title:string; url:string; location?:string; seniority?:string; source?:string;
    postedAt?: string; createdAt: string; company?: Company;
};
type ApiResult = { data: Job[]; meta: { total:number; page:number; limit:number; pageCount:number; hasPrev:boolean; hasNext:boolean } };
...
export default function JobsBrowser() {
    ...
    const [sort, setSort] = useState<'postedAt:desc' | 'postedAt:asc' | 'createdAt:desc' | 'title:asc' | 'company:asc'>('postedAt:desc');
    ...
    useEffect(() => { load(); }, [title, company, onlyStudent, dateFrom, dateTo, sort, page, limit]);

    // reset page when filters change substantively
    useEffect(() => { setPage(1); }, [title, company, onlyStudent, dateFrom, dateTo, sort, limit]);

    function changeSort(col: 'title' | 'company' | 'postedAt' | 'createdAt') {
        setPage(1);
        setSort(prev => {
            const [fld, dir] = prev.split(':');
            if (fld === col) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid md:grid-cols-6 gap-3">
                <input className="md:col-span-2 border rounded p-2 bg-transparent" placeholder="Filter by title"
                       value={title} onChange={e=>setTitle(e.target.value)} />
                <input className="md:col-span-2 border rounded p-2 bg-transparent" placeholder="Filter by company"
                       value={company} onChange={e=>setCompany(e.target.value)} />
                <input type="date" className="border rounded p-2 bg-transparent" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} />
                <input type="date" className="border rounded p-2 bg-transparent" value={dateTo} onChange={e=>setDateTo(e.target.value)} />

                <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={onlyStudent} onChange={e=>setOnlyStudent(e.target.checked)} />
                    Only Working Student
                </label>

                <select className="border rounded p-2 bg-transparent"
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    ...
                    {!loading && data.length === 0 && (
                        <tr><td colSpan={6} className="py-4 text-center text-gray-500">No results</td></tr>
                    )}
                    {data.map(j => (
                        <tr key={j.id} className="border-b hover:bg-white/5">
                            <td className="py-2 pr-3">
                                ...
                                {j.url && (
                                    <a href={j.url} target="_blank" rel="noreferrer" className="ml-2 text-xs opacity-70">
                                        ↗
                                    </a>
                                )}
                            </td>
                            ...
                            <td className="py-2 pr-3">{j.postedAt ? new Date(j.postedAt).toLocaleDateString() : '—'}</td>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="space-x-2">
                    <button className="px-3 py-1 border rounded disabled:opacity-50"
                            disabled={!meta?.hasPrev} onClick={()=>setPage(p=>Math.max(1,p-1))}>Prev</button>
                    <button className="px-3 py-1 border rounded disabled:opacity-50"
...
function Th({ label, col, sort, onClick }:{
    label:string; col?: 'title'|'company'|'postedAt'|'createdAt'; sort?: string; onClick?:(c:any)=>void;
}) {
    let arrow = '';
    if (col && sort) {
        const [fld, dir] = sort.split(':');
