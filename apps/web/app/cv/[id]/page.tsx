...
type Job = { id:number; title:string; url?:string; rawText?:string; postedAt?:string; company?: Company };
...
type Profile = { name?:string; headline?:string; summary?:string; skills?:string; projects:Project[]; experiences:Experience[] };
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                if (token) {
                    const mr = await fetch(`${apiBase}/auth/me`, { headers: { Authorization: `Bearer ${token}` } });
                    if (mr.ok) {
    ...
    return (
        <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
                <button className="text-sm opacity-70 underline" onClick={() => router.back()}>&larr; Back</button>
                <h1 className="text-2xl font-bold">{job.title}</h1>
                <div className="text-sm text-gray-400">
                    {[job.company?.name, job.postedAt ? new Date(job.postedAt).toLocaleDateString() : ''].filter(Boolean).join(' • ')}
                </div>
                {job.url && (
                    <a className="underline text-sm" href={job.url} target="_blank" rel="noreferrer">Open original posting ↗</a>
                )}
            ...
            <div className="space-y-4">
                ...
                {!me && (
                    <div className="text-sm text-yellow-400">
                        Not logged in — you can still generate, but your saved profile won’t load. <a className="underline" href="/login">Login</a>
                    </div>
                )}
                ...
                <section>
                    ...
                    {profile.projects.length === 0 ? (
                        <p className="text-sm text-gray-500">No projects. Add some in your <a className="underline" href="/profile">Profile</a>.</p>
                    ) : (
                            ...
                            {profile.projects.map((p, i) => (
                                <li key={i} className="border rounded p-2 flex items-start gap-3">
                                    <input
                                        ...
                                        className="mt-1"
                                        checked={!!pickProj[i]}
                                        onChange={(e)=>setPickProj(prev => prev.map((v, idx)=> idx===i ? e.target.checked : v))}
                                    />
                                    <div>
                                        <div className="font-medium">{p.name}</div>
                                        <div className="text-sm text-gray-400">{[p.link, p.tech].filter(Boolean).join(' • ')}</div>
                ...
                <section>
                    ...
                    {profile.experiences.length === 0 ? (
                        <p className="text-sm text-gray-500">No experience. Add some in your <a className="underline" href="/profile">Profile</a>.</p>
                    ) : (
                        <ul className="space-y-2">
                            {profile.experiences.map((x, i) => (
                                <li key={i} className="border rounded p-2 flex items-start gap-3">
                                    <input
                                        ...
                                        className="mt-1"
                                        checked={!!pickExp[i]}
                                        onChange={(e)=>setPickExp(prev => prev.map((v, idx)=> idx===i ? e.target.checked : v))}
                                    />
                                    <div>
                                        <div className="font-medium">{x.title} @ {x.company}</div>
                                        <div className="text-sm text-gray-400">{[x.start, x.end || 'Present'].filter(Boolean).join(' — ')}</div>
