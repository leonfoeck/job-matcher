...
export async function detectPersonio(company: string, website?: string): Promise<PersonioMatch | null> {
...
export async function fetchPersonioJobs(company: string, match: PersonioMatch) {
    ...
    const positions =
        ...
        doc?.position ?? [];
    const arr: any[] = Array.isArray(positions) ? positions : (positions ? [positions] : []);
    const toText = (v:any) => (typeof v === 'string' ? v.trim() : v == null ? '' : String(v).trim());
    ...
    const jobs = arr.map((p) => {
        ...
        const url = id ? `https://${match.slug}.jobs.personio.de/job/${id}` : toText(p?.url || p?.absolute_url);

        // description is HTML in most feeds
        const rawText = htmlToText(p?.description || p?.jobDescription || '');
        console.log(rawText)

        return { company, source: 'personio' as const, title, url, location, seniority, postedAt, rawText };
    });
