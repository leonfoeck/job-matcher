...
export async function detectGreenhouse(company: string, website?: string): Promise<GreenhouseMatch | null> {
...
async function mapWithLimit<T, R>(items: T[], limit: number, fn: (x: T, i: number) => Promise<R>) {
...
export async function fetchGreenhouseJobs(company: string, match: GreenhouseMatch) {
    const r = await fetchWithTimeout(match.apiUrl, 8000);
    if (!r.ok) return [];
    const data = await r.json();

    const jobs = (data.jobs ?? []).filter((j: any) => j?.title && j?.absolute_url);
