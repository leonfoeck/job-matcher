        ...
        for (const c of companies) {
            ...
            try {
                ...
                if (m) {
                    ...
                    results.push({ company: name, provider: 'personio', slug: m.slug, count: jobs.length });
                    matched = true;
                }
            } catch (e) { results.push({ company: name, provider: 'personio', error: String(e) }); }
            ...
            try {
                ...
                if (m) {
                    ...
                    console.log(jobs)
                    await this.jobs.upsertMany(jobs as any);
                    results.push({ company: name, provider: 'greenhouse', board: m.board, count: jobs.length });
                    matched = true;
                }
            } catch (e) { results.push({ company: name, provider: 'greenhouse', error: String(e) }); }
            ...
            try {
                ...
                if (m) {
                    ...
                    total += jobs.length;
                    await this.jobs.upsertMany(jobs as any);
                    results.push({ company: name, provider: 'lever', account: m.account, count: jobs.length });
                    matched = true;
                }
            } catch (e) { results.push({ company: name, provider: 'lever', error: String(e) }); }

            if (!matched) results.push({ company: name, provider: null, count: 0, note: 'no provider detected' });
